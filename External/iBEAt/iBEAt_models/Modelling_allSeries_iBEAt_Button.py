...
def main(weasel):
    ...
    filename_log = datetime.datetime.now().strftime('%Y%m%d_%H%M_') + weasel.DICOMFolder.split('/')[-1] + "_ModelAuto_LogFile.txt"
    file = open(filename_log, 'a')
    file.write(str(datetime.datetime.now())[0:19] + ": " + weasel.DICOMFolder)
    file.write("\n" + str(datetime.datetime.now())[0:19] + ": Model Auto Button started!")
    
    file.close()

    filename_csv = datetime.datetime.now().strftime('%Y%m%d_%H%M_') + weasel.DICOMFolder.split('/')[-1]+'.csv'
    ...
    studyDescription = weasel.unique_elements(weasel.series()[0]['StudyDescription'])[0]
    site = studyDescription.split('-')[-1]
    subject = weasel.DICOMFolder.split('/')[-1]
