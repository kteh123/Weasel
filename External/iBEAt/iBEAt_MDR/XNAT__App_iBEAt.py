...
def download(weasel):
    try:
            ...
            if projectName:
                ...
                if subjectName:
                    if subjectName == "All":
                        ...
                        weasel.message("Download completed!", "XNAT Download")
                    else:
        ...
        if downloadFolder != '':
            weasel.DICOMFolder = os.path.join(downloadFolder, dataset.label)
            weasel.read_dicom_folder()
            # Select/Check everything (every item) in TreeView
        ...
        weasel.information("Weasel couldn't connect to the XNAT portal because either the login details are incorrect or it requires VPN connection to the portal. See Activity_Log.log for more details")
        if "Could not determine if login was successful!" in str(e):
            weasel.information("The login details inserted might be incorrect!", "XNAT Download")
                ...
                if subjectName:
                    if subjectName == "Upload at Project Level":
                        ...
                        try:
                            ...
                            weasel.message('The zip file being uploaded contains files already present in the selected image session and the upload assistant cannot overwrite or give the option to not overwrite. \n The selected file or folder was pre-archived in the selected XNAT project. \n Please login to the portal and review and/or archive the images.')
