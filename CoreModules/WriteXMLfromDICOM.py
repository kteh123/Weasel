...
def makeDICOM_XML_File(weasel, scan_directory):
    ...
    try:
        ...
        if scan_directory:
            ...
            dictionary = build_dictionary(scans, weasel)
            xml = open_dicom_to_xml(dictionary, scans, paths, weasel)
            fullFilePath = create_XML_file(weasel, xml, scan_directory)
...
def build_dictionary(list_dicom, weasel):
    try:
        logger.info("WriteXMLfromDICOM.build_dictionary called")
        weasel.progress_bar(
            msg = "<H4>Reading data from each DICOM file</H4>",
            max = len(list_dicom))
        xml_dict = {}
        fileCounter = 0
...
def open_dicom_to_xml(xml_dict, list_dicom, list_paths, weasel):
    ...
    try:    
        ...
        fileCounter = 0
        weasel.progress_bar(
            msg = "<H4>Writing DICOM data to an XML file</H4>",
            max = len(list_dicom))
        for index, dicomfile in enumerate(list_dicom):
            fileCounter += 1
            weasel.update_progress_bar(index = fileCounter)
...
def create_XML_file(weasel, DICOM_XML_object, scan_directory):
